...
const CombinedProjects: React.FC = () => {
  ...
  // Auto-scroll every 4 seconds for carousel mode (disabled on mobile)
  useEffect(() => {
    if (!isAutoPlaying || viewMode !== 'carousel' || isMobile) return;
  ...
  }, [isAutoPlaying, viewMode, nextSlide, isMobile]);
  ...
  const getCardStyle = (index: number) => {
    if (isMobile) {
      ...
      if (diff === 0) {
        ...
        return {
          transform: `translateX(${diff > PROJECTS.length / 2 ? '-' : ''}100vw) scale(0.9)`,
          opacity: 0,
          zIndex: 1
        };
      }
  ...
  return (
    <section className="py-10 md:py-20 px-4 bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-8 md:mb-16">
          ...
          <p className="text-base md:text-xl text-gray-300 max-w-3xl mx-auto mb-6 md:mb-10 leading-relaxed px-2">
          ...
          <div className="flex justify-center mb-6 md:mb-12">
            <div className="inline-flex p-1 bg-gray-800/70 backdrop-blur-lg rounded-xl border border-gray-700">
              <button
                onClick={() => setViewMode('carousel')}
                className={`px-3 md:px-6 py-2 md:py-3 rounded-lg text-sm font-semibold transition-all duration-300 ${
                  viewMode === 'carousel'
        ...
        {viewMode === 'carousel' ? (
                      ...
                      <div className={`flex flex-col flex-grow ${isMobile ? 'p-4' : 'p-6'}`}>
                        ...
                        <div className="flex flex-wrap gap-2 mb-4">
                          {project.tags.slice(0, isMobile ? 3 : project.tags.length).map((tag) => (
                            <span
                              key={tag}
                              className={`px-2 py-1 text-xs font-semibold rounded-lg bg-gray-700/50 border border-gray-600/50 text-gray-300 ${
                                isMobile ? 'text-xs' : ''
                              }`}
                            >
                              {tag}
          ...
          <div className="relative max-w-5xl mx-auto">
                ...
                <div className={`flex flex-col justify-center ${isMobile ? 'p-6' : 'p-8 lg:p-12'}`}>
                  ...
                  <div className="flex flex-wrap gap-2 md:gap-3 mb-6 md:mb-10">
                    {currentProject.tags.map((tag) => (
                      <span
                        key={tag}
                        className={`bg-gray-700/50 backdrop-blur-sm text-gray-200 rounded-xl border border-gray-600/50 font-medium hover:bg-gray-600/50 transition-all duration-200 ${
            ...
            <div className="flex justify-between items-center mt-6 md:mt-8">
              <button
                onClick={prevSlide}
                className={`flex items-center gap-2 bg-gray-800/70 hover:bg-gray-700/70 backdrop-blur-lg border border-gray-700 rounded-lg text-gray-300 hover:text-white transition-all duration-200 ${
